...
export default async function EventRegistrationSection({ params }: { params: Promise<{ slug: string }> }) {
    ...
    <main className="py-16 md:py-32 w-full flex flex-col items-center space-y-6 max-md:mt-[-4rem]">
      <section className="container px-4 md:px-6">
        <div className="mx-auto mb-8 max-w-2xl text-center">
          <h2 className="mb-4 text-3xl md:text-4xl font-semibold lg:text-6xl">{event.title}</h2>
          <p className="mb-6 text-sm md:text-base font-medium text-muted-foreground lg:text-lg">{event.description}</p>
      ...
      <section className="container mt-8 md:mt-14 px-4 md:px-6">
        <h3 className="text-2xl md:text-3xl font-semibold mb-6 md:mb-8 text-center">Event Details</h3>
        <div className="grid gap-6 md:gap-8 md:grid-cols-2 lg:grid-cols-3">
          ...
          />
          <EventDetailCard icon={<MapPin className="h-5 w-5" />} title="Venue" content={event.venue || "To be announced"} />
          <EventDetailCard
...
function EventDetailCard({ icon, title, content }: { icon: React.ReactNode; title: string; content: React.ReactNode }) {
  return (
    <Card className='bg-accent border-background'>
      ...
      <CardContent>
        <div className="text-sm md:text-base text-muted-foreground">{content}</div>
      </CardContent>
    </Card>
  );
}
