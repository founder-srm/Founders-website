            ...
            <div className="group relative overflow-hidden rounded-xl">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent">
                <div className="absolute bottom-0 p-6 text-white">
                  <div className="mb-2 inline-flex items-center rounded-full bg-primary px-2.5 py-0.5 text-xs font-semibold text-primary-foreground">{events[0].type}</div>
            ...
            <Link key={event._id} href={`/events/writeup/${event.slug}`} className="group flex flex-col">
              <div className="mb-4 flex overflow-clip rounded-xl md:mb-5">
                <div className="h-full w-full transition duration-300 group-hover:scale-105">
                  <Image src={urlFor(event.image || "/placeholder.svg").url()} alt={event.title || ""} width={600} height={400} className="aspect-[3/2] h-full w-full object-cover object-center" />
                </div>
              </div>
              <div>
              ...
              <div className="mb-2 line-clamp-3 break-words pt-4 text-lg font-medium md:mb-3 md:pt-4 md:text-2xl lg:pt-4 lg:text-3xl">{event.title}</div>
              <div className="mb-4 line-clamp-2 text-sm text-muted-foreground md:mb-5 md:text-base">{event.summary}</div>
              <div className="flex items-center gap-2">
                <span className="relative flex shrink-0 overflow-hidden rounded-full size-12">
                  <Image className="aspect-square h-full w-full" src={urlFor(event.author?.image || "/placeholder.svg").url()} alt={event.author?.name || ""} width={48} height={48} />
                </span>
