...
export function BarChartComponent({ data = [] }: BarChartProps) {
  const processedData = React.useMemo(() => {
    ...
    const eventTypeCount = data.reduce((acc, event) => {
    ...
    <Card>
      ...
      <CardContent>
        <ChartContainer config={chartConfig}>
          <BarChart data={processedData}>
            ...
            <Bar dataKey="featured" fill="var(--color-featured)" radius={[4, 4, 0, 0]} />
      ...
      <CardFooter className="flex-col items-start gap-2 text-sm">
        <div className="flex gap-2 font-medium leading-none">
          {processedData.length} different event types <TrendingUp className="h-4 w-4" />
        </div>
